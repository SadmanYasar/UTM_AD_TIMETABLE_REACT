...
export const Route = createFileRoute('/timetable_pensyarah')({
    component: Component,
    beforeLoad: async ({ location }) => {
        if (!isAuthenticated() && getUser()?.user_auth?.role !== 'Lecturer') {
            throw redirect({
                to: '/login',
                search: {
...
export default function Component() {
    ...
    const { data: subjects } = useQuery({
        queryKey: ['subjects'],
        queryFn: () => getSubjectsByLecturer(getUser()?.user_auth?.no_pekerja?.toString() as string),
    })
