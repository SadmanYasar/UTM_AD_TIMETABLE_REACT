...
import { LecturerSubject, getSubjectsByLecturer } from "@/services/pensyarah_subjek"
...
export const Route = createFileRoute('/lecturer_timetable')({
    component: ({ params }) => <Component lecturerId={params.lecturerId} />,
...
function Component({ lecturerId }: { lecturerId: string }) {
    ...
    const { data: subjects } = useQuery({
        queryKey: ['subjects', lecturerId],
        queryFn: () => getSubjectsByLecturer(lecturerId),
    })

    const { data: times, error, isLoading } = useQuery({
        queryKey: ['timetable', lecturerId],
        queryFn: async () => {
            let largestSemesterForTheLatestSesi = 0;
            const sesi = subjects?.reduce((acc: LecturerSubject | undefined, curr: LecturerSubject) => {
            ...
            if (sesi) {
                largestSemesterForTheLatestSesi = subjects?.reduce((acc: number, curr: LecturerSubject) => {
